...
**NO FIXTURE IS EXEMPT** (2026-08-31). There were two, and both were fixtures live while
...
the loop now, so `AcceptsInput` is the whole rule for anything that touches the world, and
`ControlHintDisplay` needs no special case. **A new fixture that wants to act outside the loop is
re-opening a door that was closed twice** — check that it really cannot live inside it.

**The rule is about FIXTURES, and two UI verbs sit deliberately outside it**: PAUSE (a player who
cannot open the menu cannot reach the page that would fix it) and **SUBTITLES** (half of what the PA
says happens past `RunOver`, so a caption key on the loop's gate would be dead for the whole ending).
Neither touches the world, neither is recorded, and each is read in exactly one place. **That is the
whole list** — anything that changes the room belongs inside the gate.
...
- **THE PA SPEAKS ENGLISH IN EVERY LANGUAGE, AND IS SUBTITLED** (2026-09-02, by request). There is
  ...
  `Caption("pa.<key>")` beside any new line or a Korean player hears it with nothing on screen.
  `PaSubtitle` draws it, **and whether it is drawn is now a SETTING, on by default in every
  language** (2026-09-03, by request; it was English-off for a day). `GameSettings.Subtitles` is the
  one answer, reachable from the settings page and from **M** (`GameAction.Subtitles`) in the game.
  The `Caption` call is still required either way; what is gated is the drawing, in `PaSubtitle`
  alone. **There is no plate behind it** — contrast is an `Outline` and a `Shadow` on the glyphs,
  because one caption has to survive a white room, a dark shaft and open sky.
  This is the signage rule below, not an exception to it: the PA is the facility talking to
  itself. **The voice is ElevenLabs (`eleven_multilingual_v2`, voice "Gwen"), generated by
  `Tools/generate_narration.py`** - the third engine, after Windows SAPI and MeloTTS. **The WAVs are
  committed, so a clone builds and plays with no API key**; a key is needed only to CHANGE what the
  PA says. It is a PAID plan and the licence terms have not actually been read - `docs/asset-licences.md`
  carries the two questions to answer before the store page. `docs/audio.md`.
