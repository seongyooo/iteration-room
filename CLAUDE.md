...
- **THE PA SPEAKS ENGLISH IN EVERY LANGUAGE, AND IS SUBTITLED** (2026-09-02, by request). There is
  ...
  `PaSubtitle` draws it, **and only when the player is NOT on English** (2026-09-02, by request) - it
  is a subtitle, and an English player was reading a copy of what they had just heard. The
  `Caption` call is still required either way; what is gated is the drawing, in `PaSubtitle` alone.
