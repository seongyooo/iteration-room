...
- `Room/FinalRoomSequence.cs` — Room4, past the last door. Raises the plinth, waits on the plate, then seals the door back to Room3, announces the break, glitches every display and fades `ERROR` onto four walls before handing to `EndingSequence`. **The player keeps control until the press.** See `loop-and-ui.md`.
