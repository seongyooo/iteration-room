...
namespace IterationRoom.EditorTools
{
    ...
    public static partial class SceneBuilder
    {
        ...
        private static void BuildMainMenuScene()
        {
            ...
            // **AND THE BUILDING GOES OUT ALONG IT, A ROOM AT A TIME.** The same shot with rooms
            // painted dark, laid over the lit one at an alpha `MenuFlicker` cuts between - see that
            // script for why a photograph cannot flicker any other way, and why the pattern is hard
            // cuts rather than a fade.
            //
            ...
            if (darkShot != null)
            {
                ...
                // **AND ONE IMAGE PER STAGE OF THE FAILURE.** The frames are CUMULATIVE - frame k
                // is rooms 0 through k out - so exactly one is ever up and the flicker never has to
                // stack or order them. Siblings of the legacy dark frame
                ...
                if (frames.Count < 2)
                    Debug.LogWarning($"[SceneBuilder] Only {frames.Count} cumulative dark frame(s) on "
                                   + "disk, so the title screen's fault will blink one room instead "
                                   + "of taking the corridor room by room.");
            }
            else
            {
        ...
        // **ONE SETTINGS PAGE, BUILT WHEREVER IT IS ASKED FOR** (2026-09-05, by request: reach the
        // settings from inside a run).
        //
        // It used to be inline in `BuildMainMenuScene`, which is why the pause menu grew its own
        // loose copy of four of the six rows and `PauseMenu` grew its own copy of every handler
        ...
        private static SettingsPanel BuildSettingsPage(Transform parent, Color ink)
        {
            ...
            panel.koreanInk = koreanInk;
            panel.subtitlesOnButton = subtitlesOnButton;
            panel.subtitlesOffButton = subtitlesOffButton;
            panel.subtitlesOnInk = subtitlesOnInk;
            panel.subtitlesOffInk = subtitlesOffInk;
            panel.bindings = bindings;
            panel.backButton = settingsBack;
            return panel;
        }
