...
namespace IterationRoom
{
    ...
    public class PanelMessage : MonoBehaviour
    {
        ...
        private float alpha;
        // The announcement fires once per session, where the walls light up every visit. A sign is
        // a sign - it can sit there being true - but a voice repeating an instruction the player
        // has already followed, on every one of the several iterations they spend crossing this
        // room, would become the most irritating thing in the prototype. It is a plain field rather
        // than PlayerPrefs, so it resets when you press Play again.
        ...
        private void Update()
        {
            ...
            alpha = Mathf.MoveTowards(alpha, inside ? 1f : 0f, fadeSpeed * Time.unscaledDeltaTime);
