...
namespace IterationRoom
{
    ...
    public class CarryableItem : MonoBehaviour, IInteractHintTarget
    {
        ...
        // **THIS OBJECT DECIDES WHERE IT IS WHILE IT IS HELD, so nothing may infer its position from
        // the hand.** True for the mirrors, whose `Mirror.Sync` puts the glass in front of the
        // HOLDER'S BODY at a fixed height rather than at the hand anchor - see there for why the
        // optics need that. `HeldItemClearance` reads this and does not freeze the view for such an
        // object, because the contact it measures is not the contact the player can see.
        public bool posesItself;
