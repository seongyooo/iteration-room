...
1. **Play-test the whole run, start to finish.** This is the first build that can be *finished* and nothing in it has been played. Three things code cannot settle: whether three pads in a triangle read as "all of them, at once" without text; whether the ghosts converge in a way that makes the final walk-through feel earned rather than like waiting; and whether the ~6-iteration floor is satisfying or a grind. Known pressure point: whichever ghost bursts the key balloon does so at the same timestamp forever, and every Room3 setup iteration sits behind that.
